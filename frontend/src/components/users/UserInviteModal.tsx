...
//   );
// }
// src/pages/admin/UserInviteModal.tsx
import { useEffect, useMemo, useState } from "react";
import Modal from "../../components/ui/Modal";
import { inviteUser } from "../../api/users";
import { listClients, type ClientLite } from "../../api/client";
...
export default function UserInviteModal({
  ...
  const submit = async () => {
    if (!email || !name || roles.length === 0) {
      alert("Email, name, and roles are required");
      return;
    }
    if (needsClient && !clientId) {
      alert("Select a client for CLIENT/FIELD_EXEC");
      return;
    }
    setSaving(true);
    try {
      await inviteUser({
        email,
        name,
        password: password || undefined, // backend will generate a temp if empty
        roles,
        client_id: needsClient ? clientId : undefined,
      });
      reset();
